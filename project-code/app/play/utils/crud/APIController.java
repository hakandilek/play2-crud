...
public abstract class APIController<K, M> extends Controller {

	protected final ALogger log = Logger.of(getClass());

	protected final DAO<K, M> dao;

	public APIController(DAO<K, M> dao) {
		super();
		this.dao = dao;
	}
	...
	public Result update(K key) {
		...
		try {
			...
			return internalServerError(toJson(ImmutableMap.of(
					"status", "OK",
					"key", key,
	...
	protected Result checkRequired(String... params) {
		...
		if (!missing.isEmpty()) {
			return badRequest(toJson(ImmutableMap.of(
					"status", "missing", 
					"message", "Missing parameters :" + missing.toString())));
		}
		return null;
	}
	...
	public static Result invalid(String res) {
		...
		return notFound(toJson(ImmutableMap.of(
				"status", "not found", 
				"message", "resource not found:" + method + " " + resource)));
	}
